...
class DetectionModule(threading.Thread):
    def __init__(self, data_signal_obj, detection_queue, output_queue, base_temperature=25.0, threshold=5.0, filename="./temp_app_test2/_data/detected_values.txt"):
        super().__init__()
    ...
    def log_anomaly(self, current_time, values):
        ...
        if len(self.safety_high_temp_dict['danger']):
            ...
            # print(2, strength)
        # 파일에 누적 횟수도 함께 기록
        self.total_log = f"{current_time} avg: {average:.2f}°C max: {max:.2f}°C 강도:{strength}\n" 
