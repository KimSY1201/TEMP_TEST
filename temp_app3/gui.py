...
class DataSignal(QObject):
    update_connection_status = pyqtSignal(bool, int)
...
class OutputModule(QWidget):
    ...
    def __init__(self, data_signal_obj, available_rect):
        ...
        self.is_connected_tcp = False
        self.is_connected_232 = False
        ...
        # 1. 오디오 플레이어 객체를 여기서 한 번만 생성합니다.
        self.player = QMediaPlayer()
        self.audio_output = QAudioOutput()
        self.player.setAudioOutput(self.audio_output)
        
        # 볼륨 초기 설정 (0.0 ~ 1.0 사이의 값)
        self.audio_output.setVolume(0.5) # 50% 볼륨
        
        # (선택) 재생이 끝나면 자동으로 상태를 관리하기 위한 시그널 연결
    ...
    #     # 재생이 끝나면 (EndOfMedia), 다시 재생할 수 있도록 준비
    #     if status == QMediaPlayer.MediaStatus.EndOfMedia:
    #         print("재생 완료")
            # 필요하다면 여기서 플레이어를 정지 상태로 만들 수 있습니다.
            # self.player.stop()
    ...
    def init_ui(self):
        ...
        self.scd_status_widget = QWidget()
        self.scd_status_layout = QHBoxLayout(self.scd_status_widget)
        self.scd_status = QLabel('scd:')
        self.scd_status_layout.addWidget(self.scd_status)
        
        heat_source_grid.addWidget(self.scd_status_widget, 0, 0, alignment=Qt.AlignmentFlag.AlignCenter)
    ...
    def _create_connection_panel(self):
        ...
        layout = QHBoxLayout(connection_panel)
    ...
    def update_connection_status(self, connected, port, error_msg=""):
        """연결 상태 업데이트 (외부에서 호출)"""
        self.is_connected_tcp = connected
        self.is_connected_232 = connected
    ...
    def update_display(self, data_package):
        ...
        safety_status = data_package.get('safety_status', 'N/A')        
        ...
        if values:
            ...
            if safety_status:
                ...
                self.scd_status.setText(safety_status)
                self.scd_status_widget.setObjectName(f'{safety_status}_alert')
                # if safety_status == 'danger':
                #     # self.danger_widget.setObjectName('danger_alert')
                #     # self.caution_widget.setObjectName('caution')
                #     # self.safety_widget.setObjectName('safety')
                #     self.scd_status.setObjectName('danger_alert')
                # elif safety_status == 'caution':
                #     self.scd_status.setObjectName('caution_alert')
                #     # self.danger_widget.setObjectName('danger')
                #     # self.caution_widget.setObjectName('caution_alert')
                #     # self.safety_widget.setObjectName('safety')
                # elif safety_status == 'safety':
                #     self.scd_status.setObjectName('safety_alert')
                #     # self.danger_widget.setObjectName('danger')
                #     # self.caution_widget.setObjectName('caution')
                #     # self.safety_widget.setObjectName('safety_alert')
    ...
    def alarm_call(self, sound_name: str): # 💡 변수명을 str -> sound_name 으로 변경
        """지정된 이름의 알람 사운드를 재생합니다."""
        
        # 1. 이미 다른 사운드가 재생 중이면 추가로 재생하지 않음
        if self.player.playbackState() == QMediaPlayer.PlaybackState.PlayingState:
            return

        # 2. 재생할 사운드 파일 경로 설정 및 존재 여부 확인
        file_path = os.path.join(self.current_dir, "_asset", "sound", f"{sound_name}.wav")
        
        if not os.path.exists(file_path):
            print(f"오류: 사운드 파일을 찾을 수 없습니다 - {file_path}")
            return

        # 3. 기존에 생성된 player 객체를 재사용하여 새로운 소스 설정 및 재생
        self.player.setSource(QUrl.fromLocalFile(file_path))
        self.player.play()
        # print(f"'{sound_name}' 알람 재생 시작 (볼륨: {self.audio_output.volume() * 100:.0f}%)")
    ...
    #         file_path = f"{self.current_dir}/_asset/sound/{str}.wav"  
    #         # print(file_path)
    #         self.player.setSource(QUrl.fromLocalFile(file_path))
