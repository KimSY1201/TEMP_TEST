...
# - GUI로 내보내는 페이로드는 dashboard_temp_only.py 의 update_from_detector()와 호환
# -----------------------------------------------------------------------------
...
class DetectionModule(threading.Thread):
    ...
    큐 입력 (detection_queue) 예:
        {
          "time": "HH:MM:SS",
          "values": [64개의 온도값],
          "sensor_degree": 24.3 (옵션),
          "etc": [...](옵션)
        }
    ...
    GUI로 내보내는 페이로드 (output_queue.put) 예:
        {
          ...
          "time": "HH:MM:SS",
          "values": [... 64개 ...],
          "processed_values": [... interp*interp ...],
    ...
    def __init__(
        self,
        data_signal_obj,          # GUI가 가진 signal 객체 (parameter_update_signal 사용)
        detection_queue,
        output_queue,
        base_temperature: float = 25.0,
        threshold: float = 5.0,
        filename: str = "./temp_app/_data/detected_values.txt",
        sensor_id: int = 1,       # 이 Detector가 담당하는 센서 번호 (1~5 등)
    ):
        super().__init__()
        ...
        self.weight_list = [3.8, 3.9, 4.0, 5.1]            # center용 4단계
        self.weight_list_corner = [5, 5, 5, 5, 5, 5, 5, 5] # corner용 8단계
    ...
    def detect_suspect_fire(self, values):
        ...
        if len(self.suspect_fire_buffer) == self.deque_size:
            flattened = [item for t in self.suspect_fire_buffer for item in t[0]]
            vc = pd.Series(flattened).value_counts()
    ...
    def detect_fire(self, values) -> str:
        """
        최종 상태를 '정상'|'주의'|'화재'로 반환.
        """
        self.high_temp_counter = int(np.sum(values > min(self.filter_temp, 33)))
        ...
        if (self.high_temp_counter > 0 and self.buffer_counter == 0) and \
           self.high_temp_counter != len(self.safety_high_temp_dict):
            ...
            self.high_temp_counter_init = int(np.sum(values > min(self.filter_temp, 33)))
        ...
        if (self.high_temp_counter != self.safety_high_temp_counter) and \
           len(self.fire_detection_buffer) == self.deque_size:
            is_fire, last_coordinate = self.is_fire(self.fire_detection_buffer)
            if is_fire:
    ...
    def is_fire(self, deque_obj):
        """
        deque_obj: 최근 프레임들의 값 배열을 담은 deque
        """
        heat_source_limit = 6
        thr = min(self.filter_temp, 33)
        last_coordinate = np.where(deque_obj[-1] > thr)[0]

        # 단일 프레임에서 일정 개수 초과시 곧바로 화재
        if int(np.sum(deque_obj[-1] > thr)) > heat_source_limit:
            return True, last_coordinate

        # 앞/뒤 평균 비교로 증가 추세 판단
        if deque_obj:
            frontlist, backlist = [], []
            half = deque_obj.maxlen // 2
            dq = deque(deque_obj)  # 원본 보존
            for _ in range(half):
                if len(dq) <= 1:
                    break
                frontlist.append(int(np.sum(dq.popleft() > thr)))
                backlist.append(int(np.sum(dq.pop() > thr)))
            if not frontlist or not backlist:
                return False, last_coordinate
            front_avg = sum(frontlist) / len(frontlist)
            back_avg = sum(backlist) / len(backlist)
            return (back_avg > front_avg), last_coordinate
    ...
    def run(self):
        while self.running:
            ...
            if "values" not in data_package or "time" not in data_package:
                print("[Detector] invalid data_package: need keys 'values' and 'time'")
                continue

            values = data_package["values"]
            if len(values) != 64:
                print(f"[Detector] Expected 64 values, got {len(values)}. Skipping.")
                continue

            # (선택) ML inference (미사용 결과는 버림)
            if self.is_model_loaded and hasattr(self, "scaler") and hasattr(self, "encoder"):
                try:
                    _ = self._model_predict_silent(data_package)
                except Exception:
                    pass
    ...
    # 내부용: 조용한 모델 예측 (결과 미사용)
    def _model_predict_silent(self, data_package):
        temp_dict = {
            "sensor_degree": [data_package.get("sensor_degree", 0.0)],
            "mean": [np.mean(data_package["values"])],
        }
        for i, v in enumerate(data_package["values"]):
            temp_dict[f"thermal_{i:02d}"] = [v]
        ddf = pd.DataFrame(temp_dict)
        ddf.iloc[:, :] = self.scaler.transform(ddf.iloc[:, :])
        model = self.model["rfm"]
        _ = model.predict(ddf.iloc[:, :])
        return _
