...
class OutputModule(QWidget):
    ...
    def init_ui(self):
        ...
        # port 그룹 생성 (상호 배타적 선택 보장)
        self.port_button_group = QButtonGroup(self)
        self.com3_port = QRadioButton("COM3", self)
        self.com4_port = QRadioButton("COM4", self)
        
        self.port_button_group.addButton(self.com3_port, 0)
        self.port_button_group.addButton(self.com4_port, 1)
        
        # 기본 선택 
        self.com3_port.setChecked(True)
        self.com3_port.toggled.connect(self.position_toggled)
        self.com4_port.toggled.connect(self.position_toggled)
        
        port_layout.addWidget(self.com3_port)
        port_layout.addWidget(self.com4_port)
        ...
        left_panel_layout.addLayout(port_layout)
    ...
    def port_toggled(self):
        """포트 번호 라디오 버튼 토글 시 호출"""
        if self.com3_port.isChecked():
            self.port_num = 3
            print(f'port changed to: COM{self.port_num}')
        elif self.com4_port.isChecked():
            self.port_num = 4
            print(f'port changed to: COM{self.port_num}')
